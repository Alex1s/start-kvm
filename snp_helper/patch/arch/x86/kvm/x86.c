...
static int vcpu_run(struct kvm_vcpu *vcpu)
{
	...
	pr_info("arch/x86/kvm/x86.c vcpu_run: executing ...\n");
	...
	for (;;) {
		...
		if (kvm_vcpu_running(vcpu)) {
			pr_info("arch/x86/kvm/x86.c vcpu_run: loop is running vcpu_enter_guest ...\n");
		...
		pr_info("arch/x86/kvm/x86.c vcpu_run: loop is repeating ...\n");
		pr_info("arch/x86/kvm/x86.c vcpu_run: but we do not want that, so goodbye loop ...\n");
		break;
	}
